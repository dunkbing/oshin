...
struct ResourceLinkContent: Codable, Sendable {
...
enum EmbeddedResourceType: Codable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
                debugDescription: "Unknown resource type: \(type)"
            )
        }
    }
...
struct EmbeddedTextResourceContents: Codable, Sendable {
    let type: String = "text"
    let text: String
    let mimeType: String?
    let uri: String
...
struct EmbeddedBlobResourceContents: Codable, Sendable {
